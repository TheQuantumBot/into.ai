...
type Footer_btnProps = React.ButtonHTMLAttributes<HTMLButtonElement> & {
  children: React.ReactNode;
};

export default function Footer_btn({
  children,
  className = "",
  ...props
}: Footer_btnProps) {
  return (
    <button
      className={`relative overflow-hidden px-5 py-3 min-w-[150px] flex items-center justify-center gap-2 
        text-gray font-semibold text-lg tracking-wide rounded-lg bg-white font-inter outline-none border-[2px] border-[#DED8D3]
